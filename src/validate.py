...
#     # 严格版本
#     print(y_true, y_predict)
#     if np.setdiff1d(np.array(y_true), np.array(y_predict)).size == 0:
#         score += 1
#     # if np.intersect1d(np.array(y_true), np.array(y_predict)).size > 0:
#     #     score += 1

print(f"正确个数：{score} \t得分:{(score/len(val_dataset))*100 : .3f}")
